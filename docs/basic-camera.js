import { P as PerspectiveCamera, V as Vector3 } from "./three.js";
(function polyfill() {
...
class BasicCamera extends AssetBase {
  constructor(entities) {
    super();
    this.addPropertyPosition(true);
    this.addPropertyRotation(true);
    console.log("camera", this.viewerWidth, this.viewerHeight);
  ...
  setViewerSize(width, height) {
    this.getScene().aspect = width / height;
...
const digoAssetData = {
  ...
  create: (entities) => {
    return new BasicCamera(entities || []);
  }
};
console.log("Basic camera loaded");
