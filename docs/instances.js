import { M as MathUtils, a as Material, U as Uniform, T as Texture, C as Color, S as Scene, B as BoxGeometry, b as Mesh, I as InstancedMesh, c as BufferGeometry, d as MeshPhysicalMaterial, W as WebGLRenderTarget, O as OrthographicCamera, e as ShaderMaterial, P as PlaneGeometry, D as DoubleSide, f as Object3D, g as InstancedBufferAttribute } from "./three.js";
(function polyfill() {
...
const tokenize$1 = /* @__PURE__ */ getDefaultExportFromCjs(string);
...
const stringify = /* @__PURE__ */ getDefaultExportFromCjs(glslTokenString);
...
const tokenFunctions = /* @__PURE__ */ getDefaultExportFromCjs(glslTokenFunctions);
...
var CustomShaderMaterial = /* @__PURE__ */ function(_THREE$Material) {
  ...
  _createClass(CustomShaderMaterial2, [{
    ...
    value: function parseShader(shader) {
      ...
      var variables = stringify(tokens.slice(0, mainIndex >= 0 ? funcs[mainIndex].outer[0] : void 0));
...
const defaultProperties = {
  ...
  uniforms: {
    ...
    firstState: new Uniform(new Texture()),
    secondState: new Uniform(new Texture()),
    fboTexture: new Uniform(null),
    lightColor: new Uniform(new Color("#ffe9e9")),
    colorOne: new Uniform(new Color("#06082D")),
    colorTwo: new Uniform(new Color("#020284")),
    colorThree: new Uniform(new Color("#0000ff")),
    colorFour: new Uniform(new Color("#71c7f5")),
    aoMap: new Uniform(new Texture()),
    speed: new Uniform(0.1),
    frequency: new Uniform(1),
    displacement: new Uniform(1),
    waveDisplacement: new Uniform(1)
  },
  other: {
    objectId: "",
    firstStateId: "",
...
class GeneralData extends AssetGeneralData {
  constructor() {
    ...
    this.gridInstances = new InstancedMesh(new BufferGeometry(), new CustomShaderMaterial({ baseMaterial: new MeshPhysicalMaterial() }), this.properties.grid.numberX * this.properties.grid.numberY);
    this.fbo = {
      renderTarget: new WebGLRenderTarget(1e3, 1e3),
  ...
  updateGrid() {
    const w = this.properties.grid.space + this.properties.grid.size;
    const x = this.properties.grid.numberX;
    const y = this.properties.grid.numberY;
    const count = x * y;
    this.gridInstances.count = count;
    const dummy = new Object3D();
    const instanceUV = new Float32Array(count * 2);
    let instanceIndex = 0;
    for (let i = 0; i < x; i++) {
      for (let j = 0; j < y; j++) {
        instanceUV.set([i / x, j / y], instanceIndex * 2);
        dummy.position.set(
          w * (i - x / 2),
          0,
          w * (j - y / 2)
    ...
    this.gridInstances.geometry.setAttribute("instanceUV", new InstancedBufferAttribute(instanceUV, 2));
  }
  setupInstances() {
  ...
  setupFBO() {
    this.fbo.material.vertexShader = vertex_default;
    this.fbo.material.fragmentShader = fragment_default;
    this.fbo.material.uniforms = {
      uProgress: this.properties.uniforms.progress,
      uState1: this.properties.uniforms.firstState,
      uState2: this.properties.uniforms.secondState,
      uFBO: this.properties.uniforms.fboTexture
    };
    this.fbo.mesh.geometry = this.fbo.geometry;
    this.fbo.mesh.material = this.fbo.material;
    this.fbo.scene.add(this.fbo.mesh);
  }
  setupMaterial() {
    this.properties.uniforms.aoMap.value = this.properties.other.material.aoMap;
    const uniforms = {
      time: this.properties.uniforms.time,
      uFBO: this.properties.uniforms.fboTexture,
      aoMap: this.properties.uniforms.aoMap,
      light_color: this.properties.uniforms.lightColor,
      ramp_color_one: this.properties.uniforms.colorOne,
      ramp_color_two: this.properties.uniforms.colorTwo,
      ramp_color_three: this.properties.uniforms.colorThree,
      ramp_color_four: this.properties.uniforms.colorFour,
      uSpeed: this.properties.uniforms.speed,
      uFrequency: this.properties.uniforms.frequency,
      uDisplacement: this.properties.uniforms.displacement,
      uWaveDisplacement: this.properties.uniforms.waveDisplacement
    };
    this.gridInstances.material.onBeforeCompile = (shader) => {
      ...
      shader.vertexShader = shader.vertexShader.replace(
        ...
        float n = cnoise(vec3(instanceUV.x*uFrequency, instanceUV.y*uFrequency, time*uSpeed));

        transformed.y += n*uDisplacement;

        vHeightUV = clamp(position.y*2.,0.,1.);
        vec4 transition = texture2D(uFBO, instanceUV);
        transformed *=(transition.g);
        transformed.y += transition.r*uWaveDisplacement;
...
class Instances extends DigoAssetThree {
  constructor(entities) {
    super();
    this.webGLRenderer = Helper.getGlobal().getThreeWebGLRenderer();
    this.setLabels(labels);
    this.addDefaultProperties(true, false);
    this.addPropertyObject3D(GENERAL_PROPERTY, "geometry").setter((data, value) => {
      this.updateGeometry(data, value);
    }).getter((data) => data.properties.other.objectId);
    this.addPropertyMaterial(GENERAL_PROPERTY, "material", {}).setter((data, value, property) => this.updateMaterial(data.gridInstances, data.properties.other, "material", property, value)).getter((data) => data.properties.other.material);
    this.addPropertyNumber(GENERAL_PROPERTY, "numberX", 1, 100, 0, 1, defaultProperties.grid.numberX).setter((data, value) => {
      data.properties.grid.numberX = value;
      data.updateGrid();
    }).getter((data) => data.properties.grid.numberX);
    this.addPropertyNumber(GENERAL_PROPERTY, "numberY", 1, 100, 0, 1, defaultProperties.grid.numberY).setter((data, value) => {
      data.properties.grid.numberY = value;
      data.updateGrid();
    }).getter((data) => data.properties.grid.numberY);
    this.addPropertyNumber(GENERAL_PROPERTY, "size", 0.01, 100, 2, 0.01, defaultProperties.grid.size).setter((data, value) => {
      data.gridInstances.geometry.scale(1 / data.properties.grid.size, 1 / data.properties.grid.size, 1 / data.properties.grid.size);
      data.properties.grid.size = value;
      data.gridInstances.geometry.scale(data.properties.grid.size, data.properties.grid.size, data.properties.grid.size);
      data.updateGrid();
    }).getter((data) => data.properties.grid.size);
    this.addPropertyNumber(GENERAL_PROPERTY, "space", 0, 100, 2, 0.01, defaultProperties.grid.space).setter((data, value) => {
      data.properties.grid.space = value;
      data.updateGrid();
    }).getter((data) => data.properties.grid.space);
    this.addPropertyNumber(GENERAL_PROPERTY, "progress", 0, 1, 2, 0.01, defaultProperties.uniforms.progress.value).setter((data, value) => {
      data.properties.uniforms.progress.value = value;
    }).getter((data) => data.properties.uniforms.progress.value);
    this.addPropertyImage(GENERAL_PROPERTY, "firstState").setter((data, value) => {
      data.properties.other.firstStateId = value;
      loadImage(value).then((texture) => {
        data.properties.uniforms.firstState.value = texture;
      });
    }).getter((data) => data.properties.other.firstStateId);
    this.addPropertyImage(GENERAL_PROPERTY, "secondState").setter((data, value) => {
      data.properties.other.secondStateId = value;
      loadImage(value).then((texture) => {
        data.properties.uniforms.secondState.value = texture;
      });
    }).getter((data) => data.properties.other.secondStateId);
    this.addPropertyNumber(GENERAL_PROPERTY, "speed", 0, 100, 2, 0.01, defaultProperties.uniforms.speed.value).setter((data, value) => {
      data.properties.uniforms.speed.value = value;
    }).getter((data) => data.properties.uniforms.speed.value);
    this.addPropertyNumber(GENERAL_PROPERTY, "frequency", 0, 100, 2, 0.01, defaultProperties.uniforms.frequency.value).setter((data, value) => {
      data.properties.uniforms.frequency.value = value;
    }).getter((data) => data.properties.uniforms.frequency.value);
    this.addPropertyNumber(GENERAL_PROPERTY, "displacement", 0, 100, 2, 0.01, defaultProperties.uniforms.displacement.value).setter((data, value) => {
      data.properties.uniforms.displacement.value = value;
    }).getter((data) => data.properties.uniforms.displacement.value);
    this.addPropertyNumber(GENERAL_PROPERTY, "waveDisplacement", 0, 100, 2, 0.01, defaultProperties.uniforms.waveDisplacement.value).setter((data, value) => {
      data.properties.uniforms.waveDisplacement.value = value;
    }).getter((data) => data.properties.uniforms.waveDisplacement.value);
    this.addPropertyColor(GENERAL_PROPERTY, "colorOne", defaultProperties.uniforms.colorOne.value).setter((data, value) => {
      data.properties.uniforms.colorOne.value = new Color(value >>> 8);
    }).getter((data) => Number.parseInt(`${data.properties.uniforms.colorOne.value.getHex().toString(16)}ff`, 16));
    this.addPropertyColor(GENERAL_PROPERTY, "colorTwo", defaultProperties.uniforms.colorTwo.value).setter((data, value) => {
      data.properties.uniforms.colorTwo.value = new Color(value >>> 8);
    }).getter((data) => Number.parseInt(`${data.properties.uniforms.colorTwo.value.getHex().toString(16)}ff`, 16));
    this.addPropertyColor(GENERAL_PROPERTY, "colorThree", defaultProperties.uniforms.colorThree.value).setter((data, value) => {
      data.properties.uniforms.colorThree.value = new Color(value >>> 8);
    }).getter((data) => Number.parseInt(`${data.properties.uniforms.colorThree.value.getHex().toString(16)}ff`, 16));
    this.addPropertyColor(GENERAL_PROPERTY, "colorFour", defaultProperties.uniforms.colorFour.value).setter((data, value) => {
      data.properties.uniforms.colorFour.value = new Color(value >>> 8);
    }).getter((data) => Number.parseInt(`${data.properties.uniforms.colorFour.value.getHex().toString(16)}ff`, 16));
    const generalData = new GeneralData();
    generalData.container = new Scene();
    generalData.container.add(generalData.gridInstances);
    generalData.debug.position.y = 2;
    generalData.container.add(generalData.debug);
  ...
  updateGeometry(data, id) {
    this.loadGLTF(id, (gltf) => {
      ...
      geometry.scale(data.properties.grid.size, data.properties.grid.size, data.properties.grid.size);
      data.gridInstances.geometry = geometry;
    });
    data.properties.other.objectId = id;
  }
  tick(parameters) {
    const generalData = this.getGeneralData();
    this.webGLRenderer.setRenderTarget(generalData.fbo.renderTarget);
    this.webGLRenderer.render(generalData.fbo.scene, generalData.fbo.camera);
    this.webGLRenderer.setRenderTarget(null);
    generalData.properties.uniforms.fboTexture.value = generalData.fbo.renderTarget.texture;
    generalData.properties.uniforms.time.value = parameters.elapsedTime;
...
function loadImage(id) {
  return new Promise((resolve) => {
    ...
    image.onload = function resolveTexture() {
